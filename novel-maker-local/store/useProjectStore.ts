...
반드시 5000자 이상의 완성도 높은 소설로 작성해주세요.`;
      ...
      generateStoryboardContent: async (projectId, episodeId, cutCount) => {
        ...
        try {
          ...
          // 개선된 S급 콘티 생성 프롬프트 (완전한 출력 보장)
          const prompt = `당신은 일본 S급 만화작가(드라마틱 연출, 동적 흐름)와 한국 S급 웹툰작가(감정 몰입, 세로 스크롤 연출) 스타일을 융합한 최고의 상업 웹툰 콘티 작가입니다. 반드시 ${cutCount}컷을 모두 완성해주세요.

【절대 준수 사항】
1. 반드시 컷 1부터 컷 ${cutCount}까지 빠짐없이 모든 컷을 완성
2. 중간에 절대 끊지 말고 마지막 컷까지 완료
3. 각 컷마다 아래 형식을 정확히 준수
4. S급 연출 기법 적용 (동적 카메라 워크, 감정적 몰입, 영화적 구성)

【S급 퀄리티 연출 반영】
✦ 일본 S급 만화작가 스타일:
- 드라마틱한 구도 (로우앵글, 하이앵글, 버드아이뷰 적극 활용)
- 역동적인 움직임 표현 (모션 라인, 스피드 라인)
- 충격적인 클로즈업과 극적인 롱샷의 대비
- 감정의 절정 순간을 포착하는 타이밍
...
- 세로 스크롤에 최적화된 컷 구성
- 여백과 침묵을 활용한 심리적 연출
- 독자와의 공감대 형성을 위한 디테일한 표정 연출

【향상된 콘티 형식】
컷 [번호]: [크기/앵글] - [연출 의도]
배경: [배경 상세 묘사 + 분위기/색감/조명 설정]
인물: [인물 동작/표정/자세 상세 묘사 + 감정 상태]
대사: [인물명] "[대사 내용]" (감정: [감정 상태])
생각: [인물명] ([생각 내용]) - 폰트: [일반/기울임/굵게]
효과음: "[효과음]" (크기/위치 지정)
나레이션: "[나레이션 내용]" (분위기: [서정적/긴장감/액션])
연출 포인트: [이 컷에서 강조할 핵심 연출 요소]
...
【S급 콘티 작성 가이드라인】
• 각 컷에 여러 대사/생각이 포함될 수 있음
• 말풍선 형식: 일반, 생각(구름형), 외침(톱니형), 속삭임(점선), 나레이션(박스형)
• 감정 연출: 클로즈업으로 표정 강조, 원거리로 고독감 표현
• 액션 연출: 스피드 라인, 집중선, 충격 효과
• 심리 연출: 여백 활용, 시선 처리, 표정 변화의 미묘함

【S급 콘티 작성 시작】
지금부터 위 소설 내용을 바탕으로 일본 만화의 드라마틱한 연출과 한국 웹툰의 감정적 몰입을 융합하여 정확히 ${cutCount}컷의 최고급 상업 콘티를 작성합니다.
